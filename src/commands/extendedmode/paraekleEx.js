        ...
        const paraEmbed = new Discord.MessageEmbed()
            ...
            if (message.member.hasPermission("ADMINISTRATOR")) {
                ...
                connection.query(`SELECT * FROM users WHERE identifier = ?`,hex, (err,result) => {
                    ...
                    accounts.money += eklenecek
                    if (accounts.money >= 2147483647) return message.channel.send("Bu miktar çok büyük ekleme yapamıyorum! (Para > 2147483647)")
                    connection.query(`UPDATE users SET accounts = '${JSON.stringify(accounts)}' WHERE identifier = ?`,hex, (err,result) => {
                        ...
                        .setDescription(`${hex} ID'li oyuncunun parasına başarıyla \`${eklenecek}\` miktarı eklenmiştir. Yeni miktar \`${accounts.money}\`.`)
